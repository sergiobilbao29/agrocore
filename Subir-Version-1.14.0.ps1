# ============================================================
#  AgroCore - Publicar version 1.14.0 (Fix filtro stock hacienda por campo + fecha de endoso en cheques) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.14.0.ps1
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.14.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: fix filtro stock hacienda por campo/deposito + fecha de endoso en cheques"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Fix: el Stock de hacienda ahora respeta el filtro de deposito (si el deposito es un campo, muestra solo ese campo).`n- Cheques: nuevo campo Fecha de endoso/entrega; se completa solo al pagar con el cheque.`n- INCLUYE MIGRACION DE BASE."
