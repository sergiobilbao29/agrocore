# ============================================================
#  AgroCore - Publicar version 1.9.1 (Multimoneda Fase 1b: pago/cobro en otra moneda + diferencia de cambio) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.9.1.ps1
#  NOTA: no agrega migracion nueva (reusa los campos de v1.9.0).
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.9.1")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: Multimoneda Fase 1b - pago/cobro en otra moneda con diferencia de cambio + fix saldo pago a proveedores"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Pago/cobro de deuda en moneda extranjera: se cancela en la moneda de origen y se paga/cobra en pesos a la cotizacion del dia.`n- Diferencia de cambio automatica en ARS (ganancia/perdida) como asiento en Cuentas corrientes (Otros/Manuales).`n- Dialogo de pago/cobro muestra saldo por moneda, moneda+cotizacion del pago y preview de la diferencia.`n- Fix: pago a proveedores en cta cte usaba saldo invertido (haber-debe); ahora usa debe-haber como el resto del sistema.`n- Sin migracion nueva (reusa campos de v1.9.0)."
