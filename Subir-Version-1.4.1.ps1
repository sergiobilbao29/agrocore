# ============================================================
#  AgroCore - Publicar version 1.4.1 en GitHub (todo en uno)
#  Hacienda Fase 2 (proyeccion para banco + PDF) y acumulados.
#  Commit -> push main -> tag v1.4.1 -> Release (por API, sin 'gh').
#  Correr en la maquina FUENTE (Demo):
#     powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.4.1.ps1
# ============================================================
param(
...
# 'git add' de un archivo sin cambios es no-op, asi que es seguro listar los acumulados.
$archivos = @(
...
if ($staged) {
  $msg = "v$Version`: Hacienda Fase 2 - proyeccion de kg + valorizacion para el banco (PDF)"
  & git commit -m $msg
  Ok "Commit creado."
...
$notas = @(
  '- Hacienda Fase 2: proyeccion de kg a futuro (kg actuales + cabezas x GMD x dias) + valorizacion por precio/kg.',
  '- Estado de situacion de hacienda exportable a PDF para presentar al banco.',
  '- (Acumulado) Fase 1 hacienda + v1.2.9 cheques/tesoreria + v1.3.0 creditos/calendario.',
  '- INCLUYE MIGRACION DE BASE (de Fase 1 si no se aplico): el actualizador corre migrate deploy + db push + generate.'
) -join "`n"
