# ============================================================
#  AgroCore - Publicar version 1.22.0 (Actualizador resiliente + script de recuperacion) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.22.0.ps1
#  NOTA: no agrega migracion nueva. Publica el actualizador endurecido y el recovery.
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.22.0")
...
$archivos = @(
  ...
  "Update-AgroCore.ps1","Forzar-Actualizacion.ps1",
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: actualizador resiliente (reset --hard si pull falla) + Forzar-Actualizacion.ps1 + aviso de timeout"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Actualizador resiliente: si 'git pull --ff-only' falla (cambios locales/divergencia), fuerza la version publicada con git reset --hard y continua.`n- Nuevo Forzar-Actualizacion.ps1 para destrabar a mano una instancia colgada (backup + reset --hard + reinstalar + migrar + reiniciar, por instancia).`n- El aviso de timeout en la web indica el comando exacto.`n- IMPORTANTE: una instancia ya colgada con el actualizador VIEJO necesita correr Forzar-Actualizacion.ps1 una vez; despues se auto-cura.`n- Sin migracion nueva."
...
H1 "Listo"; Ok "v$Version publicada. En cada cliente: Verificar actualizaciones -> Instalar (las trabadas: correr Forzar-Actualizacion.ps1 una vez)."
