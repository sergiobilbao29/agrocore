# ============================================================
#  AgroCore - Publicar version 1.8.0 (Animales unificados: catalogo + productos + movimientos) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.8.0.ps1
#  NOTA: no agrega migracion nueva (no hubo cambios de schema); usa las acumuladas.
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.8.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: Animales unificados - catalogo unico de categorias, auto-productos y movimientos en Stock"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Animales unificados: las especies/categorias se administran en Catalogo -> Categorias de animales (especie, kg, peso, matriz).`n- Cada categoria es un producto del sistema (auto-creado); aparece en Stock con cabezas + kg reales.`n- Los movimientos de animales se ven en Movimientos de Stock como un producto mas; se pueden ver, editar y borrar (desde ahi o desde Hacienda).`n- Hacienda queda como vista por campo (declarado SENASA vs real) y sigue cargando movimientos por campo.`n- Selector de categoria agrupado por especie; muestra TODAS las especies y categorias.`n- Sin migracion nueva (no hubo cambios de base)."
