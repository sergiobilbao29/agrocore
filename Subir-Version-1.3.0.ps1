# ============================================================
#  AgroCore - Publicar version 1.3.0 en GitHub (todo en uno)
#  Junta lo de v1.2.8 + v1.2.9 + v1.3.0 (por si no se publicaron antes).
#  Commit -> push main -> tag v1.3.0 -> Release (por API, sin 'gh').
#  Correr en la maquina FUENTE (Demo):
#     powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.3.0.ps1
# ============================================================
param(
...
# Archivos acumulados que pueden no estar commiteados todavia.
# 'git add' de un archivo sin cambios es no-op, asi que es seguro listarlos todos.
$archivos = @(
...
if ($staged) {
  $msg = "v$Version`: creditos editar/eliminar + cuotas pagadas en bloque + alarmas editables (incluye cheques v1.2.9)"
  & git commit -m $msg
  Ok "Commit creado."
...
$notas = @(
  '- Creditos: editar y eliminar; si cambia el plan (monto/cuotas/tasa/fecha) se regenera. Boton "Marcar pagadas hasta N" para cargar creditos viejos.',
  '- Calendario: todas las alarmas editables, incluso las automaticas (al editar una auto se crea una personalizada que la reemplaza).',
  '- (Incluye v1.2.9) Cheques: estados configurables + Endosado/Entregado, campos recepcion/CUIT titular/endosante/en poder de, filtros banco/cuenta/numero/tipo, aviso 7 dias despues. Medios de pago externos. Editar/eliminar cuentas bancarias.',
