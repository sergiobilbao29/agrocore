# ============================================================
#  AgroCore - Publicar version 1.4.2 (fix alineacion tablas/PDF) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.4.2.ps1
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.4.2")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: fix alineacion de encabezados en tablas y PDFs"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Fix: alineacion de encabezados con sus valores en tablas y PDFs.`n- (Acumulado) Hacienda Fase 1+2, cheques/tesoreria, creditos/calendario.`n- INCLUYE MIGRACION DE BASE si no se aplico antes."
