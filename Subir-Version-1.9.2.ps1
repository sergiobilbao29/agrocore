# ============================================================
#  AgroCore - Publicar version 1.9.2 (Multimoneda: pago completo, dolar divisa, PDF USD, fix multi-comprobante) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.9.2.ps1
#  NOTA: no agrega migracion nueva.
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.9.2")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: Multimoneda - dialogo de pago completo, dolar divisa, factura USD por PDF, fix ruteo multi-comprobante"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Fix: en Pagar/Cobrar multi-comprobante no aparecia el comprobante recien cargado (ruteo /api/ctas-ctes/pendientes tapado por el CRUD).`n- Registrar pago/cobro por proveedor/cliente abre el dialogo completo (metodo + caja/banco/cheque + moneda + cotizacion del dia + diferencia de cambio).`n- Nueva moneda: Dolar divisa (mayorista) con cotizacion automatica.`n- Cargar factura de compra desde PDF toma moneda + cotizacion del QR de ARCA (facturas en dolares).`n- Sin migracion nueva."
