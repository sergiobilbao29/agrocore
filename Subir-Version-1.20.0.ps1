# ============================================================
#  AgroCore - Publicar version 1.20.0 (Fix Cotizaciones "Falta header X-Company-Id") + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.20.0.ps1
#  NOTA: no agrega migracion nueva.
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.20.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: fix Cotizaciones - histórico y monedas como datos globales (sin requerir X-Company-Id) + pagina resiliente"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Fix: Cotizaciones quedaba en 'Cargando...' con error 'Falta header X-Company-Id'.`n- El histórico y la lista de monedas son datos de mercado globales: ya no requieren contexto de empresa.`n- La pantalla se muestra aunque una consulta falle.`n- Sin migracion nueva."
