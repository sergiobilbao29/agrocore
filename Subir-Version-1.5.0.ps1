# ============================================================
#  AgroCore - Publicar version 1.5.0 (Hacienda Fase 3: ventas por kilo) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.5.0.ps1
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.5.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: Hacienda Fase 3 - ventas por kilo con ingreso (ctacte/efectivo/banco) y modo a rendimiento"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Hacienda Fase 3: ventas por kilo (cabezas + kg balanza + precio/kg), descuenta stock y registra el ingreso (cta cte / efectivo / banco).`n- Modo 'a rendimiento': descuenta stock ahora, cierra kg/importe y cobro despues.`n- Borrar una venta revierte el ingreso generado.`n- (Acumulado) Hacienda Fase 1+2, cheques/tesoreria, creditos/calendario, fix alineacion.`n- INCLUYE MIGRACION DE BASE."
