# ============================================================
#  AgroCore - Publicar version 1.9.0 (Multimoneda Fase 1 base: cotizaciones + facturas/cta cte en moneda) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.9.0.ps1
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.9.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: Multimoneda Fase 1 base - cotizaciones historicas + facturas/cta cte en moneda origen"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Multimoneda Fase 1 (base): pantalla Cotizaciones (Tesoreria) con dolar/euro/granos en vivo + historico diario + carga manual.`n- Facturas de compra y venta con moneda (ARS/USD/EUR/granos) y cotizacion del dia.`n- Cuenta corriente guarda la deuda en moneda origen; saldos por moneda (no mezcla pesos/dolares/granos).`n- Proximo (Fase 1b): pago en otra moneda con cotizacion del dia + diferencia de cambio automatica.`n- INCLUYE MIGRACION DE BASE."
