# ============================================================
#  AgroCore - Publicar version 1.12.0 (Catalogo de monedas editable + auto-cotizacion) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.12.0.ps1
#  NOTA: no agrega migracion nueva (las monedas propias usan el catalogo existente).
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.12.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: Catalogo de monedas editable (crear/editar/eliminar monedas propias) con auto-cotizacion"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Catalogo de monedas editable: crear/editar/eliminar monedas propias desde Tesoreria -> Cotizaciones.`n- Cada moneda propia: codigo, nombre, tipo (moneda/grano) y fuente de cotizacion automatica (dolar oficial/mayorista/blue/MEP/CCL/cripto/tarjeta o pizarra de granos) o manual.`n- Las monedas propias aparecen en facturas, cta cte, pagos/cobros y flujo de fondos, y se actualizan solas cada dia si tienen fuente.`n- Sin migracion nueva."
