# ============================================================
#  AgroCore - Publicar version 1.11.0 (Fase 3: Flujo de fondos ARS/USD con simulador) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.11.0.ps1
#  NOTA: no agrega migracion nueva.
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.11.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: Fase 3 - flujo de fondos multimoneda (ARS/USD) con simulador de escenarios y tabla mensual"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Fase 3 Flujo de fondos multimoneda: selector Ver en ARS / USD.`n- Simulador de escenarios: dolar de proyeccion + variacion de granos (%) recalculan todo el flujo.`n- Tabla mensual: Ingresos / Egresos / Saldo neto / Acumulado por mes (alerta si el acumulado queda negativo).`n- Vencimientos en USD o en toneladas de grano se valorizan correctamente (no se mezclan con pesos).`n- Sin migracion nueva."
