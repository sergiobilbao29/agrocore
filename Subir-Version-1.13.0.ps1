# ============================================================
#  AgroCore - Publicar version 1.13.0 (Fix Flujo + creditos editar/eliminar + campos como depositos) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.13.0.ps1
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.13.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: fix flujo de fondos + creditos editar/eliminar en lista + campos como depositos (con migracion)"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Fix: el Flujo de Fondos quedaba 'Cargando...' y no abria (recursion en el formato de moneda).`n- Creditos bancarios: botones Editar y Eliminar directo en la lista.`n- Campos como depositos: tilde 'Es deposito' en el alta/edicion del campo; al guardar se crea el deposito vinculado.`n- Migracion: los campos con hacienda ya cargada se convierten y vinculan a su deposito automaticamente.`n- INCLUYE MIGRACION DE BASE."
