# ============================================================
#  AgroCore - Publicar version 1.17.0 (Nueva cuenta bancaria sugiere bancos del catalogo) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.17.0.ps1
#  NOTA: no agrega migracion nueva (solo frontend).
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.17.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: nueva cuenta bancaria sugiere bancos del catalogo (datalist)"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Nueva/editar cuenta bancaria: el campo Banco sugiere los bancos del catalogo (datalist), permitiendo elegir o escribir uno nuevo.`n- Sin migracion nueva."
