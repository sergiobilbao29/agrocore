# ============================================================
#  AgroCore - Publicar version 1.15.0 (Cheques: cuenta bancaria desde las cuentas reales de Bancos) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.15.0.ps1
#  NOTA: no agrega migracion nueva (solo frontend).
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.15.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: cheques - selector de cuenta bancaria desde las cuentas reales de Bancos (filtrado por banco)"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Cheques: el selector de Cuenta bancaria ahora lista las cuentas reales cargadas en Bancos, filtradas por el banco elegido (antes tomaba un catalogo vacio).`n- Sin migracion nueva."
