# ============================================================
#  AgroCore - Publicar version 1.6.0 (Hacienda Fase 4: compras/ventas por Facturacion) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.6.0.ps1
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.6.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: Hacienda Fase 4 - compras/ventas por Facturacion (mueven stock+kg de hacienda) + vincular factura"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Hacienda Fase 4: compras/ventas de hacienda desde Facturacion (producto categoria hacienda + campo + cabezas; la cantidad de la linea son kg).`n- La factura mueve el stock de hacienda (baja/alta cabezas+kg) y maneja cobro/cta cte/ARCA; al anular se revierte.`n- Se quitaron Compra/Venta de Hacienda->Movimientos; se agrego 'Vincular factura'.`n- (Acumulado) Hacienda Fase 1-3, cheques/tesoreria, creditos/calendario, fix fechas/alineacion.`n- INCLUYE MIGRACION DE BASE."
