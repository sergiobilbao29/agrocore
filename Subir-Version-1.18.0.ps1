# ============================================================
#  AgroCore - Publicar version 1.18.0 (Columnas configurables en listados + banco solo de catalogo) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.18.0.ps1
#  NOTA: no agrega migracion nueva (solo frontend).
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.18.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: columnas configurables (mostrar/ocultar/reordenar/ordenar) en Cheques y Viajes + banco solo de catalogo en cuentas"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Columnas configurables (boton Columnas) en Cheques y Viajes: mostrar/ocultar, reordenar y ordenar por cualquier columna. Se guarda por navegador.`n- Nuevas columnas disponibles (cheques: cuenta, recepcion, endoso, CUIT titular; viajes: chofer, carta de porte, CTG).`n- Cuentas bancarias: el banco se elige del catalogo (no texto libre).`n- Sin migracion nueva."
