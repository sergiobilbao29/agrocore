# ============================================================
#  AgroCore - Publicar version 1.19.0 (Catalogo de Monedas en Catalogos) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.19.0.ps1
#  NOTA: no agrega migracion nueva (solo frontend; las monedas usan el catalogo existente).
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.19.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: pestana Monedas en Catalogos (crear/editar/eliminar monedas propias con cotizacion automatica)"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Pestana Monedas en Catalogos: predefinidas + propias, con fuente de cotizacion automatica (dolar/pizarra) o manual y ultima cotizacion.`n- El selector de moneda en compras/ventas sale de este catalogo, con cotizacion del dia.`n- 'Medios externos' es para medios de pago (Mercado Pago/billeteras), no monedas.`n- Sin migracion nueva."
