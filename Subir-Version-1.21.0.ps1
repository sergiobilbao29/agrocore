# ============================================================
#  AgroCore - Publicar version 1.21.0 (Factura PDF en U$D + monedas solo en Catalogos + web/manual actualizados) + acumulados.
#  Correr en Demo: powershell -ExecutionPolicy Bypass -File C:\AgroCore\Subir-Version-1.21.0.ps1
#  NOTA: no agrega migracion nueva. Incluye actualizacion de la web (web/index.html, web/manual.html).
# ============================================================
param([string]$InstallDir="C:\AgroCore",[string]$Version="1.21.0")
...
if (& git diff --cached --name-only) { & git commit -m "v$Version`: parser PDF U`$D + monedas administradas en Catalogos (Cotizaciones solo valores) + web/manual actualizados"; Ok "Commit creado." } else { Warn "Sin cambios staged." }
...
$notas="- Importacion de factura de compra desde PDF: reconoce dolares con 'U`$D' (ademas de U`$S/US`$/USD) y toma el total.`n- Monedas: se administran en Catalogos -> Monedas; en Cotizaciones quedan solo los valores + acceso directo.`n- Web (agrocore.ar) y Manual de Usuario actualizados con multimoneda, canje, flujo de fondos, hacienda integrada, campos-deposito y listados configurables.`n- Sin migracion nueva."
...
H1 "Listo"; Ok "v$Version publicada. En cada cliente: Verificar actualizaciones -> Instalar. La web/manual se actualizan donde sirvas agrocore.ar."
